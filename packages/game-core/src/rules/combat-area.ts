/**
 * 战斗区域判定（09 §6.1）——**唯一定义**，map / dungeon / quest / idle 共用
 *
 * ```
 * isCombatArea(map) = map != null && (map.isDungeon === true || (map.monsters?.length ?? 0) > 0)
 * ```
 *
 ...
 * `isDungeon` 非布尔真值、未知 mapKey（调用方查表失败即为 `undefined`）。
 *
