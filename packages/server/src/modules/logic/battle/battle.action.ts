...
export class BattleAction {
  ...
  async focus(ctx: FlowContext, data: unknown): Promise<ActionResult<null>> {
    ...
    // 角色归属校验（显式 key 必须等于当前角色），见 `WorldService.resolveActiveCharacter`。
    const resolved = this.world.resolveActiveCharacter(userId, body['key']);
    ...
    return guardAction(() => this.world.focus(userId, resolved.key, target));
  }
}
